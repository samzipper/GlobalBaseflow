...
source("src/BaseflowSeparationFunctions.R")
...
for (cat in 1:n.catchment){
  ...
  # figure out BFImax; Eckhardt (2008) uses >10% 0-flow days to define ephemeral
  # right now, just assuming all have porous aquifers
  if (sum(df.cat$Q_mm.d==0)/length(df.cat$Q_mm.d) > 0.1){
    BFImax <- 0.5
  } else {
    BFImax <- 0.8
  }
...
write.csv(df.summary, file.path(dir.Q.derived, "Baseflow", "catchmentSummary_Baseflow.csv"), row.names=F)
write.csv(df.summary, file.path("data", "catchmentSummary_Baseflow.csv"), row.names=F)
